...
var App = {
  ...
  ajaxCall: function(data_A){
    $.ajax({
        ...
        success: function(data){
          console.log(data);
          console.log(data_A);
          if(data.query.results.quote.BookValue === null){
            chstockApiCall = "https://chstocksearch.herokuapp.com/api/"+ data_A[3];
            secondTest = ['GET',chstockApiCall,2];
            App.ajaxCall(secondTest);
          }
          if(data.query.results.quote.BookValue !== null && data_A[2] === 1){
            var stockTicker = data.query.results.quote.symbol;
            var stockName = data.query.results.quote.Name;
            var stockBV = data.query.results.quote.BookValue;
            var stockEPS = data.query.results.quote.EarningsShare;
            var stockEBITDA = data.query.results.quote.EBITDA;
            var stockMARKCAP = data.query.results.quote.MarketCapitalization;
            var stockData = [stockTicker, stockName, stockBV, stockEPS, stockEBITDA, stockMARKCAP];
            stockDataTest = stockData;
            App.postAjaxCall(stockData, data_A[3]);
          }
          else if(data.query.results.quote.BookValue !== null && data_A[2] === 2){
            var stockTickerVal = data[0].symbol;
            var yahooApiCall = "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22" + stockTickerVal + "%22)%0A%09%09&env=http%3A%2F%2Fdatatables.org%2Falltables.env&format=json";
            var firstTest = ['GET', yahooApiCall, 1, stockTickerVal];
            App.ajaxCall(firstTest);
          }
        },
  ...
  postAjaxCall: function(data, param){
    $.ajax({
        type: "POST",
        url: "/scans?stock=" + param,
        data: {
          scan:{
            stock: data[0],
            name: data[1],
  ...
  newScan: function(){
    $("#new_scan").submit(function(e) {
      e.preventDefault();
      var stockTickerVal = $('#scan_stock').val().toUpperCase();
      var yahooApiCall = "https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.quotes%20where%20symbol%20in%20(%22" + stockTickerVal + "%22)%0A%09%09&env=http%3A%2F%2Fdatatables.org%2Falltables.env&format=json";
      var firstTest = ['GET', yahooApiCall, 1, stockTickerVal];
      App.ajaxCall(firstTest);
      var stockDataTest = [];
      //What is happening is that we are first testing whether the person put in a ticker, and then fetch it from Yahoo. If they put in a company's name, then we will do another API
      //Request to get the symbol, take the response and retest the Yahoo call with this response.
      
      console.log(stockDataTest);
    });
  },

  fundamentalClick: function(){
    console.log('fundamentalClick');
